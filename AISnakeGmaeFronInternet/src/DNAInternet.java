...
public class DNAInternet {
	...
	public DNAInternet(boolean random, int size){
		data = new byte[size];
	 ...
	 */
	public DNAInternet crossoverNoise(DNAInternet other, double mutationprob){  //byte-wise, noise applied to each value
		DNAInternet newdna = new DNAInternet(false, data.length);
	 ...
	 * Bits flip according to mutation probability
	 */
	public DNAInternet crossover(DNAInternet other, double mutationprob){
		DNAInternet newdna = new DNAInternet(false, data.length);
	 ...
	 * Bits flip according to mutation probability
	 */
	public DNAInternet crossoverBytewise(DNAInternet other, double mutationprob){
		DNAInternet newdna = new DNAInternet(false, data.length);
		int numswaps = data.length/8; 
		int swaps[] = new int[numswaps+1];
		for (int i = 0; i < swaps.length-1; i++){
